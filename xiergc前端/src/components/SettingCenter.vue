...
const saveProfile = async () => {
  try {
    ...
    const { data } = await instance.put("/user/name", {
...
const saveAccount = async () => {
  ...
  try {
    const jwtToken = localStorage.getItem("jwtToken");
    // 注意：instance 的返回结果通常在 response.data 中
    const response = await instance.patch("/user/password", {
      ...
      rePassword: account.value.confirmNewPassword
    },{
      headers: {
        Authorization: `${jwtToken}`,
      }
    ...
    // 检查返回的响应数据
    console.log("响应数据:", response);  // 调试输出完整的响应数据
    
    // 现在访问 response.data 来获取实际返回的数据
    if (response.data.code === 0) {
      alert("密码修改成功！");
      window.location.href = '/login';
      console.log("密码修改成功");
    } else {
      console.error("修改密码失败:", response.data.msg);
    }
  } catch (error) {
