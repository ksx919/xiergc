<template>
  <nav class="nav-bar">
    ...
    <div class="search-bar">
      ...
      <input class="search-input" v-model="searchValue" type="text" @keyup.enter="search" />
    </div>
    <!-- 图片按钮组 -->
    <div class="button-group">
      ...
      <a :href="button4Url" target="_blank">
        <div class="user-button" 
         @mouseenter="showDropdown = true; button4Hover = true"
         @mouseleave="showDropdown = false; button4Hover = false">
      <img :src="button4Hover ? button4SrcHover : button4Src" 
           alt="按钮4" 
           class="hoverable-button" />
      <div v-show="showDropdown" class="dropdown">
        ...
        <div class="avatar-container">
          ...
          <img :src="userAvatar" 
               ...
               v-if="userAvatar" />
          <div v-else class="avatar-placeholder">Loading...</div>
          <!-- 动态用户名 -->
          <span class="avatar-text">{{ userName || 'Loading...' }}</span>
        </div>
            <div class="user-info">
...
const fetchUserProfile = async () => {
  try {
    ...
    // 设置默认值
    userAvatar.value = require("@/assets/image/user_head.png");
    userName.value = "未知用户";
  }
};
...
const search = async () => {
  ...
  try {
    ...
    if (response.data.code === 0) {
      emit('search-result', response.data.data);
